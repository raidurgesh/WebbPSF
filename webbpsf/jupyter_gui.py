...
def show_notebook_interface_wfi(instrument):
    ...
    source_selection = widgets.Select(options=poppy.specFromSpectralType("", return_list=True), value="G0V", description="Source spectrum")
    display(source_selection)
    display(widgets.HTML(value="<hr>"))

    sca_selection = widgets.Dropdown(options=instrument.detector_list, value=instrument.detector, description="Detector:")
    sca_selection.on_trait_change(make_binding_for_attribute("detector"), name="selected_label")
    ...
    calculate_button = widgets.Button(description="Calculate PSF", width="10em", color="white", background_color="#00c403", border_color="#318732")
    display_osys_button = widgets.Button(description="Display Optical System", width="13em", color="white", background_color="#005fc4", border_color="#224A75")
    clear_button = widgets.Button(description="Clear Output", width="10em", color="white", background_color="#ed4747", border_color="#911C1C")
    ...
    def calc(*args):
        ...
        if monochromatic_toggle.value is True:
            psf = instrument.calc_psf(monochromatic=monochromatic_wavelength.value * 1e-6, display=True, outfile=OUTPUT_FILENAME, overwrite=True)
        else:
...
def show_notebook_interface_jwst(instrument):
    ...
    if instrument.name != "FGS":
        image_selection = widgets.Dropdown(options=["None"] + instrument.image_mask_list, value=str(instrument.image_mask), description="Image Mask:")

        image_selection.on_trait_change(make_binding_for_attribute("image_mask"), name="selected_label")
        display(image_selection)

        pupil_selection = widgets.Dropdown(options=["None"] + instrument.pupil_mask_list, value=str(instrument.pupil_mask), description="Pupil Mask: ")
    ...
    source_selection = widgets.Dropdown(options=poppy.specFromSpectralType("", return_list=True), value="G0V", description="Source spectrum")
    display(source_selection)
    display(widgets.HTML(value="<hr>"))
