...
class WebbOTEPupil(poppy.FITSOpticalElement):
    ...
    def __init__(self, instrument=None, level="requirements", opd_index=0, **kwargs):
        ...
        if level == "perfect":
            ...
            opd_file = os.path.join(utils.get_webbpsf_data_path(), self.instr_name, "OPD", "OPD_RevW_ote_for_{}_{}.fits".format(self.instr_name, level))
        else:
...
class NIRISS_GR700XD_Grism(poppy.AnalyticOpticalElement):
    ...
    def get_opd(self, wave):
        ...
        _log.debug(" Cylinder P-V: {0:.4g} meters optical sag at {1:.3g} microns across clear aperture".format(opd[wnz].max() - opd[wnz].min(), lambda_micron))
        return opd
...
class NIRCam_BandLimitedCoron(poppy.BandLimitedCoron):
    ...
    def __init__(self, name="unnamed BLC", kind="nircamcircular", module="A", nd_squares=True, bar_offset=None, auto_offset=None, **kwargs):
        ...
        self.offset_swb = {filt: self.get_bar_offset_from_siaf(filt, channel="SW") for filt in ["F182M", "F187N", "F210M", "F212N", "F200W", "narrow"]}
        self.offset_lwb = {
            filt: self.get_bar_offset_from_siaf(filt, channel="LW") for filt in ["F250M", "F300M", "F277W", "F335M", "F360M", "F356W", "F410M", "F430M", "F460M", "F480M", "F444W", "narrow"]
        }
    ...
    def get_bar_offset_from_siaf(self, filt, channel="LW"):
        ...
        offset_arcsec = np.sqrt(
            (self.siaf.apertures[refapername].V2Ref - self.siaf.apertures[apername].V2Ref) ** 2 + (self.siaf.apertures[refapername].V3Ref - self.siaf.apertures[apername].V3Ref) ** 2
        )
    ...
    def get_transmission(self, wave):
        ...
        if self.kind == "nircamcircular":
            ...
            if self.name == "MASKSWB":  # np.abs(self.wavelength - 2.1e-6) < 0.1e-6:
                polyfitcoeffs = np.array([2.01210737e-04, -7.18758337e-03, 1.12381516e-01, -1.00877701e00, 5.72538509e00, -2.12943497e01, 5.18745152e01, -7.97815606e01, 7.02728734e01])
            elif self.name == "MASKLWB":  # elif np.abs(self.wavelength - 4.6e-6) < 0.1e-6:
                polyfitcoeffs = np.array([9.16195583e-05, -3.27354831e-03, 5.11960734e-02, -4.59674047e-01, 2.60963397e00, -9.70881273e00, 2.36585911e01, -3.63978587e01, 3.20703511e01])
            else:
        ...
        if self.nd_squares:
            ...
            if (self.module == "A" and self.name == "MASKLWB") or (self.module == "B" and self.name == "MASK210R"):
                ...
                wnd_5 = np.where(((y < -5) & (y > -10)) & (((x > 5) & (x < 10)) | ((x < -7.5) & (x > -12.5))))
                wnd_2 = np.where(((y < 10) & (y > 8)) & (((x > 8) & (x < 10)) | ((x < -9) & (x > -11))))
            elif (self.module == "A" and self.name == "MASK210R") or (self.module == "B" and self.name == "MASKSWB"):
                # right edge
                wnd_5 = np.where(((y < -5) & (y > -10)) & (((x < 12.5) & (x > 7.5)) | ((x < -5) & (x > -10))))
                wnd_2 = np.where(((y < 10) & (y > 8)) & (((x < 11) & (x > 9)) | ((x < -8) & (x > -10))))
            else:
                # the others have two, one in each corner, both halfway out of the 10x10 box.
                wnd_5 = np.where(((y < -5) & (y > -10)) & (np.abs(x) > 7.5) & (np.abs(x) < 12.5))
                wnd_2 = np.where(((y < 10) & (y > 8)) & (np.abs(x) > 9) & (np.abs(x) < 11))
        ...
        if annotate:
            ...
            if self.name.lower() == "maskswb" or self.name.lower() == "masklwb":
                ...
                for filt, offset in offset.items():
                    ...
                    matplotlib.pyplot.text(offset + shift_dx, voffset + shift_dy, filt, color=annotate_text_color, rotation=75, horizontalalignment=horiz, verticalalignment=vert, clip_on=True)
    ...
    def __init__(self, instrument, include_oversize=False, **kwargs):
        ...
        if include_oversize:
            ...
            if self.instrument.name == "NIRISS":
                self.amplitude = fits.getdata(os.path.join(utils.get_webbpsf_data_path(), "tricontagon_oversized_4pct.fits.gz"))
                ...
                self.amplitude = fits.getdata(os.path.join(utils.get_webbpsf_data_path(), "MIRI", "optics", "MIRI_tricontagon_oversized_rotated.fits.gz"))
...
class NIRCamFieldAndWavelengthDependentAberration(WebbFieldDependentAberration):
    ...
    def __init__(self, instrument, **kwargs):
        ...
        sw_focus_cf = np.array([-5.169185169, 50.62919436, -201.5444129, 415.9031962, -465.9818413, 265.843112, -59.64330811]) / defocus_to_rmswfe
...
class MIRIFieldDependentAberrationAndObscuration(WebbFieldDependentAberration):
    ...
    def __init__(self, instrument, include_oversize=True, **kwargs):
        super(MIRIFieldDependentAberrationAndObscuration, self).__init__(instrument, include_oversize=include_oversize, **kwargs)
...
class NIRCamFieldDependentWeakLens(poppy.OpticalElement):
    ...
    def __init__(self, name="WLP8", instrument=None, center_fp_only=False, verbose=False, include_power=True, include_astigmatism=True, **kwargs):
        ...
        if center_fp_only or instrument is None:
            ...
            power_at_center_fp = {"WLM8": (-8.0188, -7.9521), "WLM4": (-4.0285, -3.9766), "WLP4": (3.9797, 3.9665), "WLP8": (8.0292, 7.9675), "WLP12": (12.0010, 11.9275)}
