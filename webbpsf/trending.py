...
def get_datetime_utc(opdhdul, return_as="string"):
    ...
    if return_as == "string":
        ...
        return astropy.time.Time(opdhdul[0].header["DATE-OBS"] + "T" + opdhdul[0].header["TIME-OBS"], format="isot", scale="utc")
    else:
...
def wavefront_time_series_plot(opdtable, start_date=None, end_date=None, ymin=0, ymax=250, threshold=80, label_visits=True, label_events=True):
    ...
    plt.plot_date(dates[where_pre][is_routine].plot_date, rms_nm[where_pre][is_routine], "o", xdate=True, label="WF Maintenance Sensing", color="C0")
    # Plot other visits (MIMF etc)
    plt.plot_date(dates[where_pre][~is_routine].plot_date, rms_nm[where_pre][~is_routine], "o", xdate=True, label="Other Sensing (MIMF etc)", color="purple")
    # Plot corrections.
    plt.plot_date(
...
def wfe_histogram_plot(opdtable, start_date=None, end_date=None, thresh=None, pid=None, download_opds=True, mark_corrections="lines", ote_only=False):
    ...
    if mark_corrections == "lines":
        ...
        axes[0].scatter(dates[where_post].plot_date, np.ones(np.sum(where_post)) * yval, marker="v", s=100, color="limegreen", label="Corrections")
        ...
        axes[0].scatter(dates[where_post].plot_date, rms_nm[where_post] + yoffset, marker="v", s=100, color="limegreen", label="Corrections", zorder=99)
    ...
    axes[0].set_title(f"{fig_title} WFE from {start_date.isot[0:10]} to {end_date.isot[0:10]}", fontsize=14, fontweight="bold")
    ...
    axes[1].set_title(f"{fig_title} WFE Histogram from {start_date.isot[0:10]} to {end_date.isot[0:10]}", fontsize=14, fontweight="bold")
    ...
    if thresh:
        for i in [1]:
            ...
            axes[i].text(xmin + 0.68 * (xmax - xmin), 0.65 * ymax, f"{fractime*100:.1f}% of the time has \nmeasured {ylabel} WFE < {thresh}", color="C2", fontweight="bold", fontsize=14)
...
def show_opd_image(array, vmax=0.300, ax=None, title=None, colorbar=False, labelrms=True, mask=None, deltatime_hrs=None, fontsize=12, maskc3=None):
    ...
    if labelrms:
        ax.text(0.02, 0.02, f"{rms(array, mask) * 1000:.1f} nm rms", color="yellow", fontweight="bold", fontsize=fontsize, verticalalignment="bottom", transform=ax.transAxes)
        if maskc3 is not None:
            ax.text(array.shape[0] - 5, 5, f"ex. C3:\n{rms(array, maskc3)*1000:.1f} nm rms", color="lightgray", fontweight="bold", fontsize=fontsize, horizontalalignment="right")
...
def single_measurement_trending_plot(opdtable, row_index=-1, reference=None, verbose=True, vmax=0.15, ignore_missing=True, subtract_target=True):
    ...
    if show_post_move:
        ...
        fig.text(0.55, 0.77, "Sensing-only visit. No mirror moves.", alpha=0.3, horizontalalignment="center", fontsize=fontsize)
    ####### Row 2
...
def cleanup_global_tt_for_wing_tilt(delta_opd, wing="left"):
    ...
    ptt_coeffs = poppy.zernike.decompose_opd_nonorthonormal_basis(delta_opd, aperture=(delta_opd != 0) & bigtiltmask, nterms=3)

    ptt_fit = poppy.zernike.compose_opd_from_basis(ptt_coeffs, poppy.zernike.zernike_basis, npix=npix, aperture=webbpsf.utils.get_pupil_mask(npix=npix))
    # Subtract that from the delta OPD
    return delta_opd - ptt_fit


def wavefront_drift_plots(opdtable, start_time, end_time, verbose=False, vmax=100, n_per_row=9, label_cid=False, label_visit=False):
    ...
    fig, axes = plt.subplots(figsize=(16, nrows * 2 + 1), nrows=nrows, ncols=n_per_row, gridspec_kw={"hspace": 0.3, "wspace": 0.01, "left": 0.01, "right": 0.93, "bottom": 0.01, "top": 0.92})
    axes_f = axes.flat
...
def monthly_trending_plot(year, month, verbose=True, instrument="NIRCam", filter="F200W", vmax=200, pid=None, opdtable=None):
    ...
    sensing_tds = astropy.time.Time(corrections_table["Post Move Sensing Time"], format="isot") - astropy.time.Time(corrections_table["Pre Move Sensing Time"], format="isot")
    ...
    min_n_im_axes = 12
    im_axes = subfigs[0].subplots(3, max(npoints, min_n_im_axes), gridspec_kw={"wspace": 0.001, "left": 0.06, "right": 0.98, "bottom": 0.02, "top": 0.8})  # , gridspec_kw={'wspace':1.000, 'left': 0})
    ...
    axes[0].fill_between([start_date.plot_date - 0.5, end_date.plot_date + 0.5], [59, 59], [80, 80], color="blue", alpha=0.08, label="Wavefront control target range")
    ...
    for i, ee_npix in enumerate([10, 2.5]):
        ...
        axes[1].plot_date(dates_array.plot_date, (ees_at_rad - median_ee) / median_ee, ls="-", color=color, label=f"$\Delta$EE within {ee_rad:.2f} arcsec ({ee_npix} pix)")

        axes[1].text(0.01, 0.75 - i * 0.12, f"Median EE within {ee_rad:.2f} arcsec = {median_ee:.3f}", color=color, fontweight="bold", transform=axes[1].transAxes)
    ...
    for i, row in enumerate(opdtable):
        if verbose:
            print(row["fileName"], row["date"], "Sensing" if row["wfs_measurement_type"] == "pre" else "Post Mirror Move", f"Obs WFE: {rms_obs[i] * 1e9:.1f} nm\tOTE WFE: {rms_ote[i]*1e9:.1f} nm")
        date = row["date"]
        ...
        if row["wfs_measurement_type"] == "pre":
            plot_index += 1
            # Plot WFS in row 1
            basic_show_image(wfes_ote[i] * 1e6, ax=im_axes[0, plot_index], nanmask=nanmask, vmax=vmax_micron)  # , title=None)
            ...
            cp = ConnectionPatch(
                [0.5, 0], (dates_array[i].plot_date, ax0ymax), coordsA="axes fraction", coordsB="data", axesA=im_axes[2, plot_index], axesB=axes[0], color="darkgreen", ls="--", alpha=0.5
            )
            fig.add_artist(cp)
    ...
    result_table = astropy.table.QTable(
        [opdtable["date"], opdtable["fileName"], wfs_type, rms_obs * 1e9 * u.nm, rms_ote * 1e9 * u.nm, ee_measurements[2.5], ee_measurements[10]],
...
def plot_phase_retrieval_crosscheck(fn, vmax_fraction=1.0):
    ...
    fig, axes = plt.subplots(figsize=(16, 9), ncols=3, nrows=2, gridspec_kw={"left": 0.10, "right": 0.90, "wspace": 0.01, "bottom": 0.02, "top": 0.85})
...
def plot_wfs_obs_delta(fn1, fn2, vmax_fraction=1.0, download_opds=True):
    ...
    plt.subplots_adjust(wspace=0.01, hspace=0.01, left=0.2, right=0.8)
    fig, axes = plt.subplots(figsize=(16, 9), ncols=3, nrows=2, gridspec_kw={"left": 0.1, "right": 0.9, "wspace": 0.01, "bottom": 0.05, "top": 0.85})
...
def show_wfs_around_obs(filename, verbose="True"):
    ...
    ax_t.text((wfe_before_dateobs + (dateobs - wfe_before_dateobs) / 2).plot_date, -0.2, f"{(dateobs-wfe_before_dateobs).to_value(u.day):.3f} days", horizontalalignment="center", color="black")
    ax_t.text((dateobs + (wfe_after_dateobs - dateobs) / 2).plot_date, -0.2, f"{(wfe_after_dateobs-dateobs).to_value(u.day):.3f} days", horizontalalignment="center", color="black")
    ...
    ax_t.text(dateobs.plot_date, 0.2, f"Science obs:\n{fnbase}", rotation=0, color="C2", horizontalalignment="right" if weight_after > weight_before else "left")
...
def show_wfs_during_program(program, verbose=False, ax=None, ref_wavefront_date=None, ref_wavefront_visit=None, start_date=None, end_date=None):
    ...
    ax.plot_date(wfs_dates_array.plot_date, rms_obs, "+", color="C1", ls="-", label="Measured RMS Wavefront Error at NIRCam NRCA3")
    ax.plot_date(wfs_dates_array.plot_date, delta_rmses, "none", color="C0", ls="--", label=f"RMS Delta Wavefront relative to {ref_label}")

    plot_sci_y = 25
    ax.scatter(science_visit_table["start_mjd"].plot_date, [plot_sci_y] * len(science_visit_table), s=150, marker="*", color="black", label=f"Program {program} observations")
    for row in science_visit_table:
...
def delta_wfe_around_time(datetime, plot=True, ax=None, vmax=0.05, return_filenames=False):
    ...
    prev_opd_fn, post_opd_fn, prev_delta_t, post_delta_t = webbpsf.mast_wss.mast_wss_opds_around_date_query(datetime, verbose=False)
...
def nrc_ta_image_comparison(visitid, verbose=False, show_centroids=False):
    ...
    if show_centroids:
        ...
        try:
            ...
            axes[0].text(
                0.95, 0.18, f"From WCS: {targ_coords_pix[0]:.2f}, {targ_coords_pix[1]:.2f}", horizontalalignment="right", verticalalignment="bottom", transform=axes[0].transAxes, color="white"
            )
        ...
        axes[0].text(
            0.95, 0.05, f" webbpsf Centroid: {cen[1]:.2f}, {cen[0]:.2f}" + oss_centroid_text, horizontalalignment="right", verticalalignment="bottom", transform=axes[0].transAxes, color="white"
        )
