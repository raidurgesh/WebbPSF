...
def mast_wss_date_query(date, tdelta):
    ...
    params = {"columns": "*", "filters": [{"paramName": "ap_type", "values": ["OPD"]}, {"paramName": "date_obs_mjd", "values": [{"min": t_start.mjd, "max": t_stop.mjd}]}]}
...
def mast_wss_opds_around_date_query(date, verbose=True):
    ...
    while len(obs_table) < 1 or min(obs_table["date_obs_mjd"]) > date.mjd or max(obs_table["date_obs_mjd"]) < date.mjd:
        if tdelta >= 6 * u.day:
            if verbose:
                print("Could not find JWST OPDs both before and after the specified date. Date outside of the available range of WFS data.")
    ...
    # In case you provide a datetime that exactly matches the datetime of an OPD file
        ...
        return (current_opd[0]["fileName"], current_opd[0]["fileName"], current_opd[0]["date_obs_mjd"] - date.mjd, current_opd[0]["date_obs_mjd"] - date.mjd)
...
def get_opd_at_time(date, choice="closest", verbose=False, output_path=None):
    ...
    if choice == "before":
        if verbose:
            print(f"User requested choosing OPD before date {date}, which is {prev_opd_fn}, delta time {prev_dtime:.3f} days")
        ...
        if verbose:
            print(f"User requested choosing OPD time closest in time to {date}, which is {closest_fn}, delta time {closest_dt:.3f} days")
...
def retrieve_mast_opd_table(aperture_list=["NRCA3_FP1"], verbose=False):
    ...
    params = {"columns": "*", "filters": [{"paramName": "ap_type", "values": ["OPD"]}, {"paramName": "apername", "values": aperture_list}]}
    ...
    columns_we_want.insert(0, astropy.table.Column(astropy.time.Time(obs_table["date_obs_mjd"], format="mjd").isot, name="date"))
...
def get_corrections(opdtable):
    ...
    correction_table = astropy.table.Table(
        ...
        names=["WFC ID", "Visit ID", "Pre Move Sensing Time", "Pre Move Sensing OPD Filename", "Post Move Sensing Time", "Post Move Sensing OPD Filename"],
    )
...
def query_wfsc_images_latest(detector="NRCA3", productlevel=3):
    ...
    # Restructuring the keywords dictionary to the MAST syntax
    params = {"columns": "filename, apername, program, observtn, visit_id, act_id,  date_obs_mjd, date_obs, productLevel", "filters": set_params(keywords)}
    service = "Mast.Jwst.Filtered.Nircam"
    tab = Mast.service_request(service, params)
...
def download_wfsc_images(program=None, obs=None, verbose=False, **kwargs):
    ...
    if verbose:
        ...
        print(f"Found {len(filetable)} level 3 data products from {filetable[0]['program']}:{filetable[0]['observtn']} around {date_obs.iso[0:16]}")
    if len(filetable) > 0:
        file_list = []
