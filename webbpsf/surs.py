...
class SegmentUpdate(object):
    ...
    def __str__(self):
        return ("Update %d, move %s, %s, %s: " % (self.id, self.segment, "absolute" if self.absolute else "relative", self.coord)) + str(self.moves)
    ...
    def xmltext(self):
        ...
        text = '        <UPDATE id="{0.id}" type="{0.type}" seg_id="{0.segment}" absolute="{absolute}" ' 'coord="{0.coord}" stage_type="{0.stage_type}">\n'.format(
            self, absolute=str(self.absolute).lower()
        )
