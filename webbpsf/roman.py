...
class WavelengthDependenceInterpolator(object):
    ...
    def set_aberration_terms(self, wavelength, zernike_array):
        ...
        if wavelength not in self._wavelengths and n_wavelengths_set < self._n_wavelengths:
            ...
            raise ValueError("Already have information at {} wavelengths " "(pass larger n_wavelengths to __init__?)".format(self._n_wavelengths))
        if len(zernike_array) != self._n_zernikes:
            raise ValueError("Expected {} aberration terms (pass different " "n_zernikes to __init__?)".format(self._n_zernikes))
    ...
    def get_aberration_terms(self, wavelength):
        ...
        if wavelength in self._wavelengths:
            ...
            if np.any(np.isnan(aberration_terms)):
                ...
                _log.warn("Attempted to get aberrations at wavelength {:.2g} " "outside the range of the reference data; clipping to closest wavelength {:.2g}".format(wavelength, wavelength_closest))
...
class FieldDependentAberration(poppy.ZernikeWFE):
    ...
    def __init__(self, pixel_width, pixel_height, name="Field-dependent Aberration", radius=1.0, oversample=1, interp_order=3):
    ...
    def get_aberration_terms(self, wavelength):
        ...
        if self.field_position in self._wavelength_interpolators:
            ...
            assert len(aberration_array.shape) == 2, "computed aberration array is not 2D " "(inconsistent number of Zernike terms " "at each point?)"
            ...
            if np.any(np.isnan(coefficients)):
                ...
                coefficients = griddata(np.asarray(field_points), np.asarray(aberration_terms), closest_interp_point, method="linear")
                ...
                if np.any(np.isnan(coefficients)):
                    ...
                    _log.warn("Attempted to get aberrations at field point {} which is outside the range " "of the reference data; approximating to nearest input grid point".format(field_position))
                else:
                    _log.warn(
                        "Attempted to get aberrations at field point {} which is outside the range " "of the reference data; approximating to nearest interpolated point {}".format(
                            field_position, closest_interp_point
                        )
                    )
                assert not np.any(np.isnan(coefficients)), "Could not compute aberration " "at field point {}".format(field_position)
...
def _load_wfi_detector_aberrations(filename):
    ...
    def build_detector_from_table(number, zernike_table):
        ...
        interpolators = {}
        detector = FieldDependentAberration(4096, 4096, radius=RomanInstrument.PUPIL_RADIUS, name="Field Dependent Aberration (SCA{:02})".format(number))
...
class WFIPupilController:
    ...
    def update_pupil(self, filter, detector):
        ...
        _log.info(f"Using {'' if self._auto_pupil_mask else 'locked '}" f"pupil mask '{pupil_mask}' and detector '{detector}'.")
...
class RomanCoronagraph(RomanInstrument):
    ...
    def fpm(self, value):
        ...
        if value.startswith("DISKSPC"):
            ...
            self._fpm_fname = os.path.join(self._datapath, "optics/DISKSPC_FPM_65WA200_360deg_-_FP1res{0:d}_evensamp_D{1:03d}_{2:s}.fits.gz".format(self._fpmres, 2 * self._Mfpm, self.filter))
        else:
            self._fpmres = 4
            self._owa = 9.0
            self._Mfpm = int(np.ceil(self._fpmres * self._owa))
            self._fpm_fname = os.path.join(self._datapath, "optics/CHARSPC_FPM_25WA90_2x65deg_-_FP1res{0:d}_evensamp_D{1:03d}_{2:s}.fits.gz".format(self._fpmres, 2 * self._Mfpm, self.filter))
    ...
    def mode(self):
        ...
        for modename, settings in self._mode_table.items():
            if self.camera == settings[0].upper() and self.filter == settings[1].upper() and self.apodizer == settings[2].upper() and self.fpm == settings[3].upper() and self.lyotstop == settings[4]:
                return modename
        return "Custom"
    ...
    def mode(self, value):
        ...
        _log.info("camera = {0}, filter = {1}, apodizer = {2}, fpm = {3}, lyotstop = {4}".format(self.camera, self.filter, self.apodizer, self.fpm, self.lyotstop))
