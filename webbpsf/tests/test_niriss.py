...
test_niriss_set_siaf = lambda: do_test_set_position_from_siaf("NIRISS", ["NIS_FP1MIMF", "NIS_SUB64", "NIS_SOSSFULL", "NIS_SOSSTA", "NIS_AMI1"])
...
def test_niriss_aperturename():
    ...
    assert niriss.detector_position == (64, 64), "Changing to a subarray aperture didn't change the " "reference pixel coords as expected"
    assert np.any(niriss._tel_coords() != ref_tel_coords), "Changing to a subarray aperture didn't change the V2V3 coords " "as expected."
