...
def test_aberration_detector_position_setter():
    ...
    assert detector._field_position == valid_pos, "Setting field position through setter did not " "update private `_field_position` value"
...
def test_WFI_detector_position_setter():
    ...
    assert wfi._detectors[wfi._detector].field_position == valid_pos, "Setting field position through Instrument.detector_position did not update field_position " "for the detector's aberration optic"
...
def test_WFI_includes_aberrations():
    ...
    assert isinstance(osys[2], roman.FieldDependentAberration), "Third plane of Roman WFI optical system should be the " "field dependent aberration virtual optic"
...
def test_swapping_modes(wfi=None):
    ...
    pupil_path = lambda self, mask=None: os.path.join(
        self._pupil_controller._pupil_basepath, self._pupil_controller.pupil_file_formatters[self._pupil_controller._get_filter_mask(self.filter) if mask is None else mask]
...
def test_WFI_limits_interpolation_range():
    ...
    assert "Requested pixel_x position" in str(excinfo.value), "FieldDependentAberration did not error on out-of-bounds field point"
    ...
    assert "Requested pixel_y position" in str(excinfo.value), "FieldDependentAberration did not error on out-of-bounds field point"
    ...
    assert allclose(det.get_aberration_terms(min_wv), det.get_aberration_terms(too_lo_wv)), "Aberration below wavelength range did not return closest value."

    assert allclose(det.get_aberration_terms(max_wv), det.get_aberration_terms(too_hi_wv)), "Aberration above wavelength range did not return closest value."
    ...
    assert np.allclose(coefficients_outlier, coefficients_data), "nearest point extrapolation " "failed for outlier field point"
...
def test_coronagraph_detector_position():
    ...
    assert "not adjustable" in str(excinfo.value), "Failed to raise exception for" "trying to change Coronagraph detector position."
