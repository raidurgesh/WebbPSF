...
test_nircam_set_siaf = lambda: do_test_set_position_from_siaf("NIRCam", ["NRCA5_SUB160", "NRCA3_DHSPIL_SUB96", "NRCA5_MASKLWB_F300M", "NRCA2_TAMASK210R"])
...
def test_nircam_auto_aperturename():
    ...
    for apname in names:
        ...
        assert nc.detector_position[0] == int(nc.siaf[apname].XSciRef), f"Aperture XSci did not match for {apname}: {nc.detector_position}, {nc.siaf[apname].XSciRef} "
        assert nc.detector_position[1] == int(nc.siaf[apname].YSciRef), f"Aperture YSci did not match for {apname}: {nc.detector_position}, {nc.siaf[apname].YSciRef} "
