...
def main():
    ...
    instruments = ["fgs", "nircam", "miri", "nirspec", "niriss"]

    # Zernike fitting order for each instrument
    order = {"fgs": 15, "nircam": 15, "miri": 15, "nirspec": 15, "niriss": 15}
    ...
    for instrument in instruments:
        ...
        for fx_index in range(0, num_fields_x):
            for fy_index in range(0, num_fields_y):
                ...
                cv_wavefront_global = basis.PointByPointWavefront(cur_data_global, data_defined_arr[fx_index, fy_index, :, :], 2, 2120, basis.units.nm, wf_basis=zernikes)
                cv_wavefront_local = basis.PointByPointWavefront(cur_data_local, data_defined_arr[fx_index, fy_index, :, :], 2, 2120, basis.units.nm, wf_basis=zernikes)
        ...
        for index in range(0, num_coeffs):
            ...
            legendre_var_local = basis.PointByPointWavefront(cur_coeffs_local, np.ones_like(cur_coeffs_local).astype(bool), 4, wavelength_nm, basis.units.nm, wf_basis=legendres)
            legendre_var_global = basis.PointByPointWavefront(cur_coeffs_global, np.ones_like(cur_coeffs_global).astype(bool), 4, wavelength_nm, basis.units.nm, wf_basis=legendres)
