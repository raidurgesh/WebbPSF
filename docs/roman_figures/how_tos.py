...
for i, filter in enumerate(sorted(all_filters)):
    ...
    c = i % long if i < long else (i % long) + 1  # remove else for left-justified bottom row
    ax = axs[r][c]
...
fig.tight_layout(w_pad=0.1, h_pad=0)  # calling twice somehow tightens h_pad
...
webbpsf.display_psf_difference(psf_sca09, psf_sca17, ax=ax_diff, vmax=5e-3, title="SCA09 - SCA17", imagecrop=2.0)
